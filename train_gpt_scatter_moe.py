...
try:
    ...
    print("Error: scattermoe not found. Please install via: pip install git+https://github.com/shawntan/scattermoe")
    sys.exit(1)
...
class MoEMLP(nn.Module):
    """
    Highly Optimized MoE MLP using ScatterMoE kernels.
    """
    ...
    def forward(self, x: Tensor):
        ...
        # Re-normalize weights so they sum to 1.0 for the computation
        topk_weights_norm = topk_weights / topk_weights.sum(dim=-1, keepdim=True)
        topk_weights_norm = topk_weights_norm.to(x.dtype)
        topk_indices = topk_indices.to(torch.int32)

        # --- 2. Computation (ScatterMoE Kernel) ---
        # x: [N, D], k_weights: [N, k], k_idxs: [N, k]
        output = self.experts(x_flat, topk_weights_norm, topk_indices)
        
        output = output.view(B, T, D)

        # --- 3. Auxiliary Loss (Load Balancing) ---
        # Switch Transformer Loss: num_experts * sum(fraction_routed * avg_prob)
        
        # Hard assignment (fraction of tokens going to each expert)
        # We look at the first chosen expert (top-1) for load balancing
        top1_index = topk_indices[:, 0] 
        expert_mask = F.one_hot(top1_index.long(), num_classes=self.num_experts).float()
        fraction_routed = expert_mask.mean(dim=0) # [NumExperts]
        
        # Soft assignment (average probability assigned to each expert)
        mean_prob = router_probs.mean(dim=0) # [NumExperts]
        
        # The Loss
        load_balancing_loss = self.num_experts * torch.sum(fraction_routed * mean_prob)
        
        # Router Z-Loss (keeps logits small)
...
# Router, gate, AND EXPERT params go to Adam (optimizer1)
